...
const patterns = {
  ...
  equipment:
    /(?:седельный\s*тягач|экскаватор|погрузчик|бетонораспределитель|самосвал|кран|бульдозер|автокран|каток|фреза|грейдер)/i,
} as const
...
function preprocessLines(text: string): string[] {
  return text
    ...
    .filter((line) => {
      ...
      return trimmed && trimmed !== "я" && trimmed.length > 1 && !isNoiseLine(trimmed)
    })
}
...
function extractPricingInfo(data: CommercialOfferData, normalizedText: string) {
  ...
  if (priceMatch) {
    ...
    if (isValidPrice(priceStr)) {
      data.price = Number.parseInt(priceStr)
    }
  }
...
function isValidSpecValue(value: string): boolean {
  ...
  const invalidValues = ["я", "характеристики", "техники", "коммерческое предложение", "стоимость", "цена"]
