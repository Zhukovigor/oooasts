...
function getVkRedirectUri() {
  if (typeof window === "undefined") {
    return process.env.NEXT_PUBLIC_VK_REDIRECT_URI || "https://asts.vercel.app/api/vk/callback";
  }

  const configuredRedirectUri = process.env.NEXT_PUBLIC_VK_REDIRECT_URI;

  if (configuredRedirectUri && !window.location.hostname.match(/^(localhost|127\.0\.0\.1)$/)) {
    return configuredRedirectUri;
  }

  return `${window.location.origin}/api/vk/callback`;
}

function isLocalHost() {
  return typeof window !== "undefined" && window.location.hostname.match(/^(localhost|127\.0\.0\.1)$/);
}
...
function VkAuthContent() {
  ...
  const vkAuthUrl = redirectUri
    ? `https://oauth.vk.com/authorize?client_id=${VK_APP_ID}&display=page&redirect_uri=${encodeURIComponent(redirectUri)}&scope=wall,photos,groups,offline&response_type=code&v=5.131`
    : "#";

  useEffect(() => {
    const resolvedRedirectUri = getVkRedirectUri();
      ...
      try {
        if (window.VKIDSDK) {
          ...
          if (authBox) {
            ...
            oAuth.render({
              container: authBox,
              oauthList: [VKID.OAuthName?.VK || "vkid"]
